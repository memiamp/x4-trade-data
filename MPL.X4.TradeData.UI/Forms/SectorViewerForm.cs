...
using static MPL.X4.TradeData.UI.Controls.PlotControl;
...
//    }
    ...
    private void Initialise()
    {
        ...
        plot.AddItem(new PlotItem(240000, 245000, ItemType.Ship, "Container Ship - MV Ocean Star"));
        plot.AddItem(new PlotItem(260000, 255000, ItemType.Plane));
    }

    private void LoadSectorData()
    {
        var maxX = _sector?.Ships.Max(x => x.Position.X) ?? 0;
        var maxZ = _sector?.Ships.Max(x => x.Position.Z) ?? 0;

        var max = Math.Max(maxX, maxZ);
