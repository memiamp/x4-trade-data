...
public interface ICatalogFileReader
{
    ...
    Task<IEnumerable<ICatalogIndexEntry>> ParseIndex(string catalogPath, int catalogId, string? fileFilter = null);
    ...
    /// <returns>A <see cref="Task{TResult}"/> representing the asynchronous operation.  Am <see cref="IEnumerable{T}"/> of <see cref="ICatalogIndexEntry"/> containing the result.</returns>
    Task<IEnumerable<ICatalogIndexEntry>> ParseIndexes(string catalogPath, string? fileFilter = null, bool recursiveSearch = false);
