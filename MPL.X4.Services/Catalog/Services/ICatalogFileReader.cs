...
public interface ICatalogFileReader
{
    ...
    Task<IEnumerable<ICatalogIndex>> GetIndex(string catalogPath, bool recursiveSearch = false);
    ...
    /// <returns>A <see cref="Task{TResult}"/> representing the asynchronous operation.  Am <see cref="IEnumerable{T}"/> of <see cref="ICatalogIndexEntry"/> containing the result.</returns>
    Task<ICatalogIndex> ParseIndex(string catalogPath, int catalogId, string? fileFilter = null, string? fileExtension = null);
    ...
    /// <param name="fileExtension">A nullable <see cref="string"/> containing the file extension to match files with.</param>
    /// <param name="recursiveSearch">A <see cref="bool"/> indicating whether to recursively search for catalog files under <paramref name="catalogPath"/>.</param>
    /// <returns>A <see cref="Task{TResult}"/> representing the asynchronous operation.  Am <see cref="IEnumerable{T}"/> of <see cref="ICatalogIndex"/> containing the result.</returns>
    Task<IEnumerable<ICatalogIndex>> ParseIndexes(string catalogPath, string? fileFilter = null, string? fileExtension = null, bool recursiveSearch = false);
    ...
    /// </summary>
    /// <param name="entries">An <see cref="IDictionary{TKey, TValue}"/> of key <see cref="string"/> that is the data file path, and value <see cref="IEnumerable{T}"/> of <see cref="ICatalogIndexEntry"/> that are the entries to read.</param>
    ...
    IAsyncEnumerable<string> ReadTextFiles(IDictionary<string, IEnumerable<ICatalogIndexEntry>> entries, CancellationToken cancellationToken = default);
}
